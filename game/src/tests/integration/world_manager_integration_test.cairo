...
mod tests {
    ...
    use dojo_starter::models::world::{Biome, Hex, HexArea, derive_area_id};
    ...
    fn world_manager_integration_discover_paths_are_stateful_and_idempotent() {
        ...
        system.discover_hex(adventurer_id, target, Biome::Forest, 6_u8);
        ...
        assert(first_hex.area_count == 6_u8, 'INT_DISCOVER_HEX_AREAS');
        assert(first_hex.biome == Biome::Forest, 'INT_DISCOVER_HEX_BIOME');
        assert(first_actor.energy == 75_u16, 'INT_DISCOVER_HEX_ENERGY');

        system.discover_hex(adventurer_id, target, Biome::Desert, 2_u8);
        let replay_hex: Hex = world.read_model(target);
        let replay_actor: Adventurer = world.read_model(adventurer_id);

        assert(replay_hex.biome == Biome::Forest, 'INT_REPLAY_HEX_BIOME');
        assert(replay_hex.area_count == 6_u8, 'INT_REPLAY_HEX_AREAS');
        assert(replay_actor.energy == 75_u16, 'INT_REPLAY_HEX_ENERGY');

        system.discover_area(
            adventurer_id, target, 0_u8, dojo_starter::models::world::AreaType::Control, 77_u16,
            dojo_starter::models::world::SizeCategory::Large,
        );
        ...
        assert(control_area.resource_quality == 77_u16, 'INT_CTRL_QUALITY');
        ...
        system.discover_area(
            second_adventurer_id,
            target,
            1_u8,
            dojo_starter::models::world::AreaType::PlantField,
            88_u16,
            dojo_starter::models::world::SizeCategory::Medium,
        );
        ...
        assert(area.resource_quality == 88_u16, 'INT_AREA_QUALITY');
        ...
        system.discover_area(
            adventurer_id, target, 1_u8, dojo_starter::models::world::AreaType::Wilderness, 1_u16,
            dojo_starter::models::world::SizeCategory::Small,
        );
        let replay_area: HexArea = world.read_model(area_id);
        assert(
            replay_area.area_type == dojo_starter::models::world::AreaType::PlantField,
            'INT_AREA_REPLAY_TYPE',
        );
        assert(replay_area.resource_quality == 88_u16, 'INT_AREA_REPLAY_QUALITY');
    }
}
