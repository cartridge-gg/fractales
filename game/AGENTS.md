...
- 2026-02-17: For inspect readability, render payload sections as structured cards/tables (not one raw `<pre>` dump) and HTML-escape every dynamic value (including tooltip attributes) so long felt IDs stay legible without unsafe markup.
- 2026-02-17: In TypeScript workspaces with `exactOptionalPropertyTypes`, avoid passing optional `RequestInit` fields as explicit `undefined` (and avoid unsupported `BodyInit` variants); build init objects incrementally so Node-side request bridges compile cleanly.
- 2026-02-17: In `ws` transports, gate sends with `WebSocket.OPEN` (static constant, not instance property), and in runtime tests attach message listeners before ingest to avoid false negatives from dropped early frames.
- 2026-02-17: For explorer live transport, bootstrap stream sessions with a `/v1/status` probe before opening `/v1/stream`, and normalize `http(s)` origins to `ws(s)` once in a shared helper to prevent environment-specific URL drift.
- 2026-02-17: In live store patch handling, track a snapshot-resync latch after `resync_required`/sequence gaps and skip data mutations until `replaceChunks` completes; this prevents stale stream patches from mutating state during catch-up windows.
- 2026-02-17: For incremental explorer rendering, keep stream patch application sequence-gated in both store and renderer and recompute app-visible selectors after each accepted patch, so UI state stays coherent without forcing chunk reloads.
- 2026-02-17: For inspect event observability, have proxy `getHex` compose `reader.getHexInspect` with a bounded `reader.getEventTail` query and sort rows by `(block, tx, event)` before returning payloads, so UI order is deterministic regardless of backend row order.
- 2026-02-17: Treat claimability as an explicit checkpoint signal (`claimable_since_block > 0`) across SQL views and client mapping; never infer it from local decay thresholds, or rendered claim state can drift from canonical world semantics.
- 2026-02-17: For scalable live reads, keep viewport chunk selection dynamic but capped (prefetch ring + hard max key budget), and cache chunk responses behind TTL in the proxy client so repeated viewport frames do not trigger unbounded `/v1/chunks` traffic.
- 2026-02-17: For live renderer migration, use an adapter that preserves stable interaction behavior via existing renderer contracts while computing WebGL-package render sets/snapshots in parallel; this de-risks transport + UI milestones before full GPU draw-path swap.
- 2026-02-17: For multi-layer readability, avoid single-winner layer colors; composite active layer signals deterministically (claims/ownership/adventurers/resources/decay) so overlapping states remain visible, and lock this with both app-level fill snapshots and renderer-pipeline composite snapshots.
- 2026-02-17: For inspect UX, keep compact mode as default and implement full mode as additive raw-field cards (not a replacement layout), so debugging depth increases without regressing readability or breaking existing inspect section affordances.
- 2026-02-17: For P5 hardening gates, keep one executable script (`scripts/p5-hardening-gate.ts`) as threshold source of truth and have CI upload its JSON report artifact; this makes perf/freshness regressions fail fast with reproducible numbers.
- 2026-02-17: For browser-hosted explorer live mode, proxy HTTP responses must include CORS headers (`Access-Control-Allow-Origin`, methods, headers) and handle `OPTIONS`, or cross-port app mounts fail with generic `Failed to fetch`.
- 2026-02-17: Keep proxy runtime defaulting to Torii-backed reader (stub only behind explicit env flag), and wire `getStatus` to sampled live chunk head; otherwise `/v1/status` can stay zero and mask real snapshot availability.
- 2026-02-17: For map navigation fidelity, pass viewport state from app to renderer (`setViewport`) and render hex coordinates in world space relative to viewport center; auto-fitting loaded hexes recenters every frame and makes pan appear broken.
- 2026-02-17: Renderer surface tiling must be viewport-derived (generate axial window from camera center/zoom), not derived from loaded chunk count; otherwise large canvases show sparse 20-hex clusters instead of full-map lattice.
- 2026-02-17: For inspect/sidebar upgrades, lock the gameplay-loop card contract first (`assignment -> active operation -> output/event`) and thread payload changes through DTO, runtime query, proxy merge, and formatter tests in one scoped plan to avoid UI drift.
- 2026-02-17: For production-feed escaping tests, keep event names canonical and inject unsafe content in payload fields; strict event-name filters can otherwise hide rows and create false-negative assertions.
- 2026-02-17: For selected-inspect live refresh, treat operation patches as coordinate-bearing payloads (`hexCoordinate`, `hex_coordinate`, `current_hex`) across nested `row/hex/claim/plant/adventurer` envelopes; relying on `hex_patch` only misses valid world updates.
- 2026-02-17: For inspect scalability gates, enforce row-cap notices via one shared capped-table helper across compact cards; otherwise sections can silently truncate without any operator-visible indicator.
- 2026-02-17: Keep sidebar regressions under a dedicated CI job (`explorer_sidebar_regression`) that runs `bun run typecheck && bun run test:app && bun run test:proxy` in `client/` so inspect contract/render/refresh drift fails quickly.
- 2026-02-17: On Slot Katana basic deployments, `block_time = 1000` + `no_mining = false` can still behave tx-driven (idle block number does not advance); always verify cadence via direct `starkli block-number` sampling before relying on block-time-based game loops.
