import { RawArgs, Calldata, CairoOption, CairoCustomEnum, RpcProvider, Contract, Account, AccountInterface, AllowArray, Call, UniversalDetails, InvokeFunctionResponse, CallResult, CallContractResponse, Abi, TypedData } from 'starknet';
import { z } from 'zod';
...
type ExtractFunctionSignature<F, ABI extends readonly any[] = never> = F extends {
    ...
    stateMutability: F extends {
        state_mutability: infer Mut;
    } ? Mut extends string ? Mut : "external" : "external";
} : never;
/**
...
type ExtractEnumVariants<Name extends string, ABI extends readonly any[]> = FindFirstEnumByName<ABI, Name> extends {
    type: "enum";
    name: Name;
    variants: infer V;
} ? V extends readonly {
    name: string;
    type: string;
}[] ? CairoEnumVariantMap<V, ABI> : never : never;
/**
 * Extract just the type union of an enum by name
 * This is used for type mapping in function parameters
 */
type ExtractEnumType<Name extends string, ABI extends readonly any[]> = ExtractEnumVariants<Name, ABI> extends infer VariantMap ? [VariantMap] extends [never] ? never : Name extends `core::option::Option::<${infer Inner}>` ? CairoOption<MapCairoType<Inner, ABI>> : CairoCustomEnum & {
...
type ModelStructNames<ABI extends readonly any[]> = Extract<ExtractStructNames<ABI>, `${string}::models::${string}`>;
type MergeModelEntries<ABI extends readonly any[]> = UnionToIntersection$1<ModelStructNames<ABI> extends infer Name ? Name extends `${infer Namespace}::models::${infer Model}` ? {
    [K in Namespace]: {
        [P in Model]: ExtractStructType<Name, ABI>;
    };
} : {} : {}>;
type ExtractModels<ABI extends readonly any[]> = Simplify<MergeModelEntries<ABI>>;
type ActionInterfaceNames<ABI extends readonly any[]> = Extract<ExtractInterfaceNames<ABI>, `${string}::systems::actions::${string}`>;
type MergeActionEntries<ABI extends readonly any[], Interfaces extends Record<string, any>> = UnionToIntersection$1<ActionInterfaceNames<ABI> extends infer Name ? Name extends `${infer Namespace}::systems::actions::${infer Interface}` ? {
    [K in Namespace]: {
        [P in Interface]: Interfaces[Name & keyof Interfaces];
    };
} : {} : {}>;
type ExtractActions<ABI extends readonly any[], Interfaces extends Record<string, any>> = Simplify<MergeActionEntries<ABI, Interfaces>>;
/**
...
type ModelsFromAbi<T> = ModelCollection<T>;
type ActionsFromAbi<T> = ActionCollection<T>;
type ModelPathUnion<Models> = Models extends Record<string, Record<string, any>> ? {
    [Namespace in keyof Models & string]: {
        [Model in keyof Models[Namespace] & string]: `${Namespace}-${Model}`;
    }[keyof Models[Namespace] & string];
}[keyof Models & string] : never;
type ModelPathFromAbi<T> = ModelPathUnion<ModelCollection<T>>;
type GetModel<T, Path extends ModelPathFromAbi<T>> = ModelCollection<T> extends Record<string, Record<string, any>> ? Path extends `${infer Namespace}-${infer Model}` ? Namespace extends keyof ModelCollection<T> ? Model extends keyof ModelCollection<T>[Namespace] ? ModelCollection<T>[Namespace][Model] : never : never : never : never;
type GetActions<T, Namespace extends keyof ActionCollection<T>> = ActionCollection<T>[Namespace];
type GetActionInterface<T, Namespace extends keyof ActionCollection<T>, InterfaceName extends keyof ActionCollection<T>[Namespace]> = ActionCollection<T>[Namespace][InterfaceName];
type GetActionFunction<T, Namespace extends keyof ActionCollection<T>, InterfaceName extends keyof ActionCollection<T>[Namespace], FunctionName extends keyof ActionCollection<T>[Namespace][InterfaceName]> = ActionCollection<T>[Namespace][InterfaceName][FunctionName];
...
type DojoActionInputs<Fn> = Fn extends {
    inputs: infer Inputs;
} ? keyof Inputs extends never ? undefined : Inputs : undefined;
...
type DojoActionInterface = Record<string, ActionSignature>;
...
type ExtractActionUnion<Actions> = Actions extends ReadonlyArray<infer Item> ? EnsureActionInterface<Item> : EnsureActionInterface<Actions>;
type NormalizeActions<Actions> = EnsureActionInterface<UnionToIntersection<ExtractActionUnion<Actions>>> extends infer Result ? Result extends never ? Record<never, ActionSignature> : Result : Record<never, ActionSignature>;
type DojoActionMethod<Fn> = Fn extends {
    stateMutability: infer Mut;
} ? Mut extends "view" ? DojoActionInputs<Fn> extends undefined ? () => Promise<CallResult> : (args: DojoActionInputs<Fn>) => Promise<CallResult> : DojoActionInputs<Fn> extends undefined ? (account: Account | AccountInterface) => Promise<InvokeFunctionResponse> : (account: Account | AccountInterface, args: DojoActionInputs<Fn>) => Promise<InvokeFunctionResponse> : DojoActionInputs<Fn> extends undefined ? (account: Account | AccountInterface) => Promise<InvokeFunctionResponse> : (account: Account | AccountInterface, args: DojoActionInputs<Fn>) => Promise<InvokeFunctionResponse>;
type DojoActionMethodMap<Actions> = {
    [K in keyof NormalizeActions<Actions> & string]: DojoActionMethod<NormalizeActions<Actions>[K]>;
};
/**
 ...
 */
declare class DojoProviderBase extends Provider {
...
type DojoProviderInstance<Actions = never> = DojoProviderBase & DojoActionMethodMap<Actions>;
type DojoProviderConstructor = new <Actions = never>(manifest?: any, url?: string, logLevel?: LogLevel) => DojoProvider<Actions>;
type DojoProvider<Actions = never> = DojoProviderInstance<Actions>;
declare const DojoProvider: DojoProviderConstructor;
 ...
 */
declare const getContractAbi: (manifest: any, contract: any) => Abi;
/**
...
export { type ActionsFromAbi, type DojoActionInterface, type DojoCall, type DojoConfig, DojoProvider, type DojoProviderInstance, type ExtractAbiTypes, type ExtractAbiTypesFromArray, type GetActionFunction, type GetActionInterface, type GetActions, type GetModel, type IWorld, KATANA_CLASS_HASH, KATANA_ETH_CONTRACT_ADDRESS, KATANA_PREFUNDED_ADDRESS, KATANA_PREFUNDED_PRIVATE_KEY, LOCAL_KATANA, LOCAL_RELAY, LOCAL_TORII, type Manifest, type MapCairoType, type ModelPathFromAbi, type ModelsFromAbi, WorldEntryPoints, createDojoConfig, createManifestFromJson, createModelTypedData, getContractAbi, getContractByName, parseDojoCall, parseModelName };
